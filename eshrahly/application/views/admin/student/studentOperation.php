...
<aside class="right-side">
    ...
    <section class="content-header">
        <h1> Student Operations<small>(List)</small></h1>
        <ol class="breadcrumb">
            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">List of Student Operations</h3>
                    </div>
                    <div class="box-body">
                            ...
                            foreach ($request as $key) { ?>
                                <tr>
                                   ...
                                   <td><?php echo $key['date']; ?></td>
                                    <td>
                                        <?php 
                                         $hours = floor($key['total_second']/ 3600);
                                        echo $hours;//gmdate("H", $key['total_second'])?>
                                        Hours</td>
                                    <td><?php $remaining_time = floor($key['remaining_time']/ 3600);//echo gmdate("H", $key['remaining_time']); ?> Hours</td>
                                    <td>
...
function getRecord(id){
    ...
    $.ajax({
    type: "GET",
    url: "<?php echo base_url('index.php/Admin/fetchStudentReqdata');?>/?from="+from+"&to="+to+"&id="+id,
      success: function(data){
       ...
       if(obj.length>0){
          ...
          for(var i=0;i<obj.length;i++){
            ...
            var payment_status = obj[i]['payment_status'];
            var lesson_status  = obj[i]['lesson_status'];
            ...
            var time             = obj[i]['time'];
            var date             = obj[i]['date'];
            var total_time       = obj[i]['total_second'];
            var remaining_time   = obj[i]['remaining_time'];
            ...
            if (lesson_status == '2') {
                lesstatus = 'Lesson Completed';
            }else{
                lesstatus = 'Lesson Pending';
            }
            j = ++j;

            html += '<tr><td>'+j+'</td><td>'+teacher_name+'</td><td>'+student_name+'</td><td>'+date+'</td><td>'+hours+' Hours</td><td>'+remaining_hours+' Hours</td><td>'+service_selector+'</td><td>'+stage+'</td><td>'+subject+'</td><td>'+service_type+'</td><td>'+time+'</td><td>'+lesstatus+'</td><td>'+getstatus+'</td></tr>';
          }
